...
from texttemplater import texttemplater as tt
...
class HelperFunctionTests(unittest.TestCase):
    ...
    def test_find_scope(self):
        text = "[[one scope]] foo {{bar}} "\
            "[[second scope [[with inner scope]] test]]"
        ...
        assertText = "[[second scope [[with inner scope]] test]]"
        self.assertEqual(assertText, text[start:end])
...
class ReplaceTemplateTests(unittest.TestCase):
    ...
    def test_replace_basic(self):
        ...
        assertText = "true and or not"
        self.assertEqual(assertText, tt.replace(template, {"p1": "true"}))

        template = "Hello {{name}}, [[you are a "\
            "{{profession}}||you have no profession]]."\
            "[[ Also you are {{adj}}!]]"
        solutions = [
        ...
        for solution, data in zip(solutions, datas):
            self.assertEqual(
                solution,
                tt.replace(template, data),
                msg="{} <=> {}".format(template, data)
            )

    def test_repace_default(self):
        template = "[[Hello [[{{prefix}}||dear]] "\
            "{{name}}||Dear ladies and gentlemen]]"
        self.assertEqual(
            "Hello dear Jens",
            tt.replace(template, {"name": "Jens"})
        )
        self.assertEqual(
            "Dear ladies and gentlemen",
            tt.replace(template, {"prefix": "value"})
        )
    ...
    def test_replace_elseblock(self):
        ...
        self.assertEqual(
            "delete else",
            tt.replace(text, {"undefined": "delete else"})
        )

        text = "[[{{very-much-nothing}} in scope1 "\
